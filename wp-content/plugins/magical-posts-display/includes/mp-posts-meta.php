...
if ( ! function_exists( 'mp_display_posted_by' ) ) :
	...
	function mp_display_posted_by() {
		...
		echo '<span class="byline"> <i class="fas fa-user-circle"></i>' . $byline . '</span>'; // WPCS: XSS OK.
	...
	function mp_display_category_link() {
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links"><i class="far fa-folder-open"></i>' . esc_html__( ' %1$s', 'magical-posts-display' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
...
function mpd_one_cat($id){
	...
	echo '<i class="far fa-folder-open"></i> <a href="'.esc_url(get_category_link($mpd_category)).'">'. esc_html($mpd_category->name).'</a>';
	}else{
