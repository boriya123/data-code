...
function mgpd_cat_list(){
    $options['all'] = __('Select category','magical-posts-display');
...
$mpd_args1 = array(
        'post_type'         =>  'post',
        'post_status'       =>  'publish',
        'posts_per_page'    => $mpdtab_posts_number,
        'tax_query'         => array(
            array(
                'taxonomy'  => 'category',
                'field'  => 'slug',
                'terms'  => $mpdtab_posts_cat_one
            )
        ),
        'orderby'        => $mg_orderby,
        'order'   => $mg_order,
...
$mpd_args2 = array(
    ...
    mpd_get_style_item(
        '.tab.mgpd-tab.mgpdtab'.esc_attr($rand_num).' .tab-content img',$mgimg_width
...
<div class="tab mgpd-tab mgpdtab<?php echo esc_attr($rand_num); ?> mgpd-style1 mb-3" role="mgptab">
        ...
        <?php if($mpdtab_posts_cat_one != 'all'): ?>
            <li class="mgtnav-item"><a href="#mgpdcat1<?php echo esc_attr($rand_num); ?>" aria-controls="cat1" role="tab" class="active" data-toggle="tab"><?php echo esc_html($mpdtab_posts_cat_one); ?></a></li>
        <?php endif; ?>
        <?php if($mpdtab_posts_cat_two != 'all'): ?>
            <li class="mgtnav-item"><a href="#mgpdcat2<?php echo esc_attr($rand_num); ?>" aria-controls="cat2" role="tab" data-toggle="tab"><?php echo esc_html($mpdtab_posts_cat_two); ?></a></li>
        <?php endif; ?>
        </ul>
                <!-- Tab panes -->
    <div class="tab-content tabs pt-3 pb-3">
        <div role="mgptab" class="tab-pane mgpdtab1 fade in active show" id="mgpdcat1<?php echo esc_attr($rand_num); ?>">
        ...
        <div role="mgptab" class="tab-pane fade" id="mgpdcat2<?php echo esc_attr($rand_num); ?>">
             <div class="mgpd mp-display-gird mgptab mgpdtab2 mgpdtab2<?php echo esc_attr($rand_num); ?> ">
                <div class="mgpd-masonry">
                <div class="row">
            <?php
                
                $mpd2_loop= new WP_Query($mpd_args2);
                if ($mpd2_loop -> have_posts() ) :
                while ( $mpd2_loop->have_posts()) :  $mpd2_loop->the_post();
                $post_thumb_src = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'large' );
                //tag list
                $tags_list = get_the_tag_list();
                ?>
